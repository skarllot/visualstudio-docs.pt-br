...
manager: "douge"
---
# Usando cobertura de c&#243;digo para determinar quanto c&#243;digo est&#225; sendo testado
...
### Para analisar a cobertura de código em testes de unidade no Gerenciador de Testes  
  
1.  No menu **Testar**, escolha **Analisar Cobertura de Código**.  
  
2.  Para ver quais linhas foram executadas, escolha ![Mostrar ícone de cor de cobertura de código](../test/media/codecoverage-showcoloringicon.png "CodeCoverage\-ShowColoringIcon")**coloração de cobertura de código mostram**.  
...
## Emissão de relatórios em os blocos ou linhas  
...
## Gerenciando os resultados da cobertura de código  
...
## Mesclando resultados de execuções diferentes  
...
## Excluindo elementos dos resultados da cobertura de código  
...
### Excluindo elementos em código C\+\+ nativo  
...
-   *SourceFilePath* é o local ou caminho UNC de um arquivo. cpp.  Pode conter curingas.  O exemplo a seguir exclui todos os arquivos em um diretório específico: `\\MyComputer\Source\UnitTests\*.cpp`  
...
### Incluindo ou excluindo elementos adicionais  
...
## Analisando a cobertura de código no serviço de compilação  
 Quando você faz check\-in de seu código, os testes serão executados no servidor de compilação, juntamente com os outros testes de outros membros da equipe.  \(Se você ainda não tiver configurado esse, consulte [Executar testes no processo de compilação](../Topic/Run%20tests%20in%20your%20build%20process.md).\) É útil analisar a cobertura de código no serviço de compilação, pois isso dá o panorama mais recente e mais abrangente da cobertura no projeto inteiro.  Também incluirá os testes automatizados do sistema e outros testes codificados que você normalmente não executa nos computadores de desenvolvimento.  
  
1.  No Team Explorer, abra **Compilações** e adicione ou edite uma definição de compilação.  
  
2.  Na página **Processo**, expanda **Testes Automatizados**, **Fonte de Teste**, **Configurações de Execução**.  Defina **Tipo de Arquivo de Configurações de Execução** para **Cobertura de Código Habilitada**.  
 ...
 ![Configurar a definição de compilação para cobertura de código](../test/media/codecoverage-plaincc.png "CodeCoverage\-plainCC")  
...
## Analisando a cobertura de código em uma linha de comando  
 Para executar testes a partir da linha de comando, use vstest.console.exe.  A cobertura de código é uma opção desse utilitário.  Para obter mais informações, consulte [Opções da linha de comando de VSTest.Console.exe](/devops-test-docs/test/vstest-console-exe-command-line-options).  
...
## Solução de Problemas  
 Se você não vir os resultados da cobertura de código, consulte  [Solução de problemas de cobertura de código](../test/troubleshooting-code-coverage.md).  
