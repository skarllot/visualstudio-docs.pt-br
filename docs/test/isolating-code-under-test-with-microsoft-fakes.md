---
title: "Isolando c&#243;digo em teste com falsifica&#231;&#227;o da Microsoft | Microsoft Docs"
ms.custom: ""
ms.date: "12/03/2016"
...
-   Um [shim](#shims) modifica o código compilado de seu aplicativo no tempo de execução para que, em vez de fazer uma chamada de método específica, ele executa o código shim que seu teste fornece.  Os shims podem ser usados para substituir chamadas para assemblies que você não pode modificar, como assemblies do .NET.  
  
 ![Falsificações substituir outros componentes](../test/media/fakes-2.png "Fakes\-2")  
  
 **Requisitos**  
  
-   O Visual Studio Enterprise  
  
## Escolhendo entre os tipos de stub e shim  
...
##  <a name="stubs"></a> Introdução aos stubs  
 Para obter uma descrição mais detalhada, consulte [Usando stubs para isolar partes de seu aplicativo para testes de unidade](../test/using-stubs-to-isolate-parts-of-your-application-from-each-other-for-unit-testing.md).  
...
2.  **Adicionar assembly do Fakes**  
    ...
    1.  No Gerenciador de Soluções, expanda a lista de referências do projeto de teste.  Se você estiver trabalhando no Visual Basic, escolha **Mostrar Todos os Arquivos** para ver a lista de referências.  
...
3.  Em seus testes, construa instâncias do stub e forneça o código para seus métodos:  
    ...
    ```  
     ...
     Os stubs também são gerados para getters e setters de propriedades, para eventos e métodos genéricos.  Para obter mais informações, consulte [Usando stubs para isolar partes de seu aplicativo para testes de unidade](../test/using-stubs-to-isolate-parts-of-your-application-from-each-other-for-unit-testing.md).  
  
##  <a name="shims"></a> Introdução aos shims  
 \(Para obter uma descrição mais detalhada, consulte [Usando shims para isolar seu aplicativo de outros assemblies para testes de unidade](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md).\)  
...
1.  **Adicionar assembly do Fakes**  
     ...
     No Gerenciador de Soluções, abra as referências de seu projeto de teste de unidade e selecione a referência ao assembly que contém o método que você deseja forjar.  Nesse exemplo, a classe `DateTime` está em **System.dll**.  Para ver as referências em projeto do Visual Basic, escolha **Mostrar Todos os Arquivos**.  
...
2.  **Inserir um shim em um ShimsContext**  
    ...
    ```  
     ...
     Os nomes das classes de shims são compostos pelo prefixo `Fakes.Shim` adicionado ao nome do tipo original.  Os nomes dos parâmetros são acrescentados ao nome do método.  \(Você não precisa adicionar qualquer referência de assembly para System.Fakes.\)  
...
```  
 ...
 \(Não há nenhum assembly de 'System.IO.Fakes' para referência.  O namespace é gerado pelo processo de criação de correção.  Mas você pode usar 'using' ou 'Import' da maneira usual.\)  
  
 Você também pode criar shims para instâncias específicas, para construtores e propriedades.  Para obter mais informações, consulte [Usando shims para isolar seu aplicativo de outros assemblies para testes de unidade](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md).  
  
## Nesta seção  
 [Usando stubs para isolar partes de seu aplicativo para testes de unidade](../test/using-stubs-to-isolate-parts-of-your-application-from-each-other-for-unit-testing.md)  
  
 [Usando shims para isolar seu aplicativo de outros assemblies para testes de unidade](../test/using-shims-to-isolate-your-application-from-other-assemblies-for-unit-testing.md)  
