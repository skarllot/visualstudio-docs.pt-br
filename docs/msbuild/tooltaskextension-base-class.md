...
|<xref:Microsoft.Build.Tasks.ToolTaskExtension.Log%2A>|Parâmetro somente leitura <xref:Microsoft.Build.Utilities.TaskLoggingHelper> opcional.<br /><br /> Obtém uma instância de uma classe <xref:Microsoft.Build.Tasks.TaskLoggingHelperExtension>que contém métodos de registro em log de tarefa.|  
