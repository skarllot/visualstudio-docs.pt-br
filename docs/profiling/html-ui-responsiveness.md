...
dev_langs: 
  ...
  - "C++"
helpviewer_keywords: 
  - "desempenho, JavaScript [aplicativos da Windows Store]"
  - "ferramentas de desempenho, JavaScript [aplicativos da Windows Store]"
  - "Criador de Perfil de Capacidade de Resposta da Interface de Usuário [JavaScript]"
  - "Criador de perfil, capacidade de resposta da interface do usuário [JavaScript]"
  - "Criador de perfil, JavaScript [aplicativos da Windows Store]"
...
Este tópico descreve como isolar problemas de desempenho em seus aplicativos usando o criador de perfil de capacidade de resposta de interface do usuário, uma ferramenta de desempenho disponível para aplicativos Windows Universal.  
...
##  <a name="RunningProfiler"></a> Executar a ferramenta de capacidade de resposta da interface do usuário HTML  
 Você pode usar a ferramenta de capacidade de resposta de interface do usuário HTML quando você tem um aplicativo Universal do Windows ou da Windows Store aberto no Visual Studio ou instalado em um computador executando o Windows 8 ou posterior.  
...
2.  Sobre o **Depurar** menu, escolha **criador de perfil de desempenho...**.  
  
     Se você quiser alterar o destino da análise para o criador de perfil, escolha**Alterar destino**.  
  
     ![Change analysis target](../profiling/media/js_tools_target.png "JS\_Tools\_Target")  
...
3.  Em **Ferramentas Disponíveis**, selecione **Capacidade de Resposta da Interface de Usuário HTML** e clique em **Iniciar**.  
  
4.  Quando você inicia o Criador de Perfis de Capacidade de Resposta de Interface de Usuário, a janela de Controle de Conta de Usuário pode solicitar sua permissão para executar o Visual Studio ETW Collector.exe. Selecione **Sim**.  
  
     Interaja com o aplicativo para testar o cenário de desempenho relevante. Para um fluxo de trabalho detalhado, consulte [Isolar um problema de capacidade de resposta da interface de usuário](#Workflow) e [Isolar um problema de taxa de transferência visual](#IsolateVisualThroughput).  
...
##  <a name="IsolateAnIssue"></a> Isolar um problema  
 A seção a seguir fornece sugestões para ajudá\-lo a isolar problemas de desempenho. Para obter uma explicação detalhada de como identificar e corrigir problemas de desempenho usando um aplicativo de teste de desempenho de amostra, consulte [Passo a passo: Melhorando a capacidade de resposta da interface de usuário \(HTML\)](../profiling/walkthrough-improving-ui-responsiveness-html.md).  
  
###  <a name="Workflow"></a> Isolar um problema de capacidade de resposta da interface de usuário  
...
4.  Como opção, adicione marcas de usuário ao seu código, usando [Marcar código para análise](#ProfileMark).  
...
7.  Alterne para o Visual Studio \(pressione Alt\+Tab\) e selecione **Parar de coletar** na guia do criador de perfil do Criador de Perfil de Capacidade de Resposta da Interface de Usuário.  
  
8.  Se já tiver adicionado marcas de usuário, elas aparecerão na [Exibir a linha do tempo da sessão de diagnóstico](#Ruler) do criador de perfil. A ilustração a seguir mostra uma única marca de usuário usada para especificar uma operação específica em seu código.  
  
     ![Diagnostics Ruler showing a user mark](../profiling/media/js_htmlvizprofiler_usermark.png "JS\_HTMLVizProfiler\_UserMark")  
...
9. Identifique uma área de interesse na linha de tempo e nos gráficos do criador de perfil usando marcas de usuário, eventos de ciclo de vida do aplicativo ou dados visíveis nos gráficos. Veja a seguir algumas diretrizes para ajudá\-lo a analisar e usar os dados dos gráficos:  
    ...
    -   Use a [Exibir a linha do tempo da sessão de diagnóstico](#Ruler) para exibir [Marcar código para análise](#ProfileMark), os eventos do ciclo de vida do aplicativo e as linhas do tempo associadas desses eventos e dos dados em outros gráficos.  
  
    -   Use o [CPU utilization graph](#CPUutilization) para exibir informações gerais sobre a atividade da CPU e o tipo de trabalho com o qual está lidando durante um determinado período. Períodos de atividade excessiva da CPU têm maior probabilidade de causar problemas de capacidade de resposta e quadros ignorados.  
  
    -   Se estiver desenvolvendo um jogo ou aplicativo de mídia avançada, use o [Exibir a taxa de transferência visual (FPS)](#VisualThroughput) para identificar períodos em que a taxa de quadros cai.  
...
10. Selecione a área de interesse em um dos gráficos clicando em uma parte e arrastando o ponteiro para fazer uma seleção \(ou usando a tecla Tab e as teclas de seta\). Quando você seleciona um período fazendo uma seleção, o gráfico de detalhes da linha do tempo no painel inferior do criador de perfis muda para mostrar somente o período selecionado.  
     ...
     ![CPU utilization graph](../profiling/media/js_htmlvizprof_cpu_util.png "JS\_HTMLVizProf\_CPU\_Util")  
  
11. Use o [Exibir detalhes da linha de tempo](#TimelineDetails) para obter informações detalhadas sobre eventos que estejam sendo executados com muita frequência ou que estejam demorando muito para serem concluídos. Por exemplo, procure o seguinte:  
    ...
    -   Outros eventos especificados na [Referência de evento do criador de perfil](#ProfilerEvents).  
...
```  
 ...
 ![User measure event in the timeline details view](../profiling/media/js_htmlvizprofiler_user_measure.png "JS\_HTMLVizProfiler\_User\_Measure")  
...
###  <a name="Ruler"></a> Exibir a linha do tempo da sessão de diagnóstico  
 ...
 ![Diagnostic session ruler](../profiling/media/js_htmlvizprof_ruler.png "JS\_HTMLVizProf\_Ruler")  
  
 A linha do tempo mostra quando ocorrem os eventos de ciclo de vida do aplicativo, como o evento de ativação, e mostra marcas de usuário \(triângulos com a marca de usuário\) que você pode adicionar ao seu código. Você pode selecionar os eventos para mostrar dicas de ferramenta com mais informações. Para mais informações sobre marcas de usuário, consulte [Marcar código para análise](#ProfileMark) neste tópico.  
...
-   Obtenha uma exibição mais detalhada de um período selecionado clicando no botão **Ampliar**.  
 ...
 Para saber mais sobre como usar o gráfico, consulte [Isolar um problema de capacidade de resposta da interface de usuário](#Workflow) neste tópico.  
...
-   O gráfico a seguir mostrará o valor zero se os quadros forem ignorados.  
 ...
 ![Visual throughput graph](../profiling/media/js_htmlvizprof_vizthru.png "JS\_HTMLVizProf\_VizThru")  
...
>  Quando ocorre um evento em um thread de plano de fundo, a ID do thread aparece entre colchetes ao lado do nome do evento.  
 ...
 ![Timeline details graph](../profiling/media/js_htmlvizprof_timelinedet.png "JS\_HTMLVizProf\_TimelineDet")  
 ...
 Os eventos no gráfico de detalhes da linha do tempo são codificados por cores para representar as mesmas categorias de trabalho mostradas no gráfico de utilização da CPU. Para obter mais informações sobre as categorias do evento e sobre os eventos específicos, consulte [Referência de evento do criador de perfil](#ProfilerEvents) neste tópico.  
...
###  <a name="FilterEvents"></a> Filtrar eventos  
 ...
 ![Filtering events in the timeline](../profiling/media/js_htmlvizprofiler_event_filter.png "JS\_HTMLVizProfiler\_Event\_Filter")  
  
 Para filtrar os eventos de solicitação HTML, desmarque a opção **Tráfego de rede** do ícone de filtro no painel inferior. Por padrão, esses eventos são mostrados no gráfico de detalhes da linha do tempo.  
  
 Para filtrar a atividade do thread da interface do usuário, desmarque a opção **Atividade da interface de usuário**.  
...
###  <a name="GroupFrames"></a> Agrupar eventos por quadro  
 ...
 ![Timeline events grouped by frame](../profiling/media/js_htmlvizprofiler_frame_grouping.png "JS\_HTMLVizProfiler\_Frame\_Grouping")  
...
-   **Carregando.** Indica o tempo gasto para recuperar recursos do aplicativo e analisar HTML e CSS quando o aplicativo é carregado pela primeira vez. Isso pode incluir solicitações de rede.  
...
-   **Estilo.** Indica o tempo gasto analisando o CSS e calculando a apresentação e o layout do elemento.  
...
-   Observação [Este vídeo](http://channel9.msdn.com/Events/Build/2013/3-316) da conferência Build 2013 sobre o criador de perfil de capacidade de resposta da interface do usuário.  
...
-   Para obter informações sobre o modelo de execução de código de thread único e o desempenho, consulte [executando código](http://msdn.microsoft.com/library/windows/apps/hh781217.aspx).  
  
## Consulte também  
 [Analisar o desempenho de aplicativos](../Topic/Analyze%20the%20performance%20of%20Windows%20Store%20apps%20using%20Visual%20Studio%20diagnostic%20tools.md)