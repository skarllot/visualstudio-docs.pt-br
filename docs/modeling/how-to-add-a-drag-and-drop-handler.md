---
title: 'Como: adicionar um manipulador de arrastar e soltar | Documentos do Microsoft'
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
-   As duas primeiras seções descrevem métodos alternativos para definir um manipulador de gestos:  
    ...
    -   [Definindo manipuladores de gestos substituindo métodos ShapeElement](#overrideShapeElement). Os métodos `OnDragDrop`, `OnDoubleClick`, `OnDragOver` e outros podem ser substituídos.  
...
-   [Como obter o Original arrastado Item](#getOriginal). Se o item arrastado for um elemento DSL, é possível abrir o modelo de origem e acessar o elemento.  
  
-   [Usando ações de Mouse: Arrastando itens de compartimento](#mouseActions). Este exemplo demonstra um manipulador de nível inferior que intercepta ações do mouse nos campos de uma forma. O exemplo permite ao usuário reordenar os itens em um compartimento arrastando com o mouse.  
  
##  <a name="a-nameoverrideshapeelementa-defining-gesture-handlers-by-overriding-shapeelement-methods"></a><a name="overrideShapeElement"></a>Definindo manipuladores de gestos substituindo métodos ShapeElement  
...
-   <xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.OnDragOver%2A>-Este método é chamado quando o ponteiro do mouse entra na forma durante uma operação de arrastar.</xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.OnDragOver%2A> O método deve inspecionar o item que o usuário está arrastando e definir a propriedade Efeito para indicar se o usuário pode soltar o item nessa forma. A propriedade Efeito determina a aparência do cursor enquanto estiver sobre a forma e também determina se `OnDragDrop()` será chamado quando o usuário soltar o botão do mouse.  
...
-   <xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.OnDragDrop%2A>– Esse método é chamado se o usuário libera o botão do mouse enquanto o ponteiro do mouse estiver sobre essa forma ou diagrama, se `OnDragOver(DiagramDragEventArgs e)` definida anteriormente `e.Effect` para um valor diferente de `None`.</xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.OnDragDrop%2A>  
...
-   <xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.OnDoubleClick%2A>– Esse método é chamado quando o usuário clica duas vezes na forma ou diagrama.</xref:Microsoft.VisualStudio.Modeling.Diagrams.ShapeElement.OnDoubleClick%2A>  
...
##  <a name="a-namemefa-defining-gesture-handlers-by-using-mef"></a><a name="MEF"></a>Definindo manipuladores de gestos usando MEF  
 MEF (Managed Extensibility Framework) permite definir componentes que podem ser instalados com configuração mínima. Para obter mais informações, consulte [Managed Extensibility Framework (MEF)](http://msdn.microsoft.com/Library/6c61b4ec-c6df-4651-80f1-4854f8b14dde).  
  
#### <a name="to-define-a-mef-gesture-handler"></a>Para definir um manipulador de gestos de MEF  
  
1.  Adicionar ao seu **Dsl** e **DslPackage** projetos a **MefExtension** arquivos que são descritos em [estender a DSL usando MEF](../modeling/extend-your-dsl-by-using-mef.md).  
...
##  <a name="a-nameextractinga-how-to-decode-the-dragged-item"></a><a name="extracting"></a>Como decodificar o item arrastado  
...
-   <xref:System.Windows.Forms.IDataObject>  `Data`– Essa propriedade contém versões serializadas dos objetos de origem, geralmente em mais de um formato.</xref:System.Windows.Forms.IDataObject> Suas funções mais úteis são:  
...
-   <xref:Microsoft.VisualStudio.Modeling.ElementGroupPrototype>`Prototype` – Use essa propriedade se desejar que os usuários arrastar itens de uma DSL ou um modelo UML.</xref:Microsoft.VisualStudio.Modeling.ElementGroupPrototype> Um protótipo de grupo de elementos contém um ou mais objetos, links e os valores de suas propriedades. Também é usado em operações colar e ao adicionar um elemento da caixa de ferramentas. Em um protótipo, objetos e seus tipos são identificados por Guid. Por exemplo, esse código permite ao usuário arrastar elementos de classe de um diagrama UML ou do Gerenciador de Modelos UML:  
...
##  <a name="a-namegetoriginala-how-to-get-the-original-of-a-dragged-element"></a><a name="getOriginal"></a>Como obter o original de um elemento arrastado  
...
1.  Torne a DSL de origem acessível pelo [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] Model Bus:  
    ...
    2.  Abra o arquivo de definição da DSL da DSL de origem no Designer de DSL. Clique na superfície de design e, em seguida, clique em **habilitar Modelbus**. Na caixa de diálogo, escolha uma ou as duas opções.  Clique em **OK**. Um novo projeto "ModelBus" é adicionado à solução de DSL.  
  
    3.  Clique em **transformar todos os modelos** e recompile a solução.  
  
###  <a name="a-namembra-to-send-an-object-from-a-source-dsl"></a><a name="mbr"></a>Enviar um objeto de uma DSL de origem  
...
##  <a name="a-namemouseactionsa-using-mouse-actions-dragging-compartment-items"></a><a name="mouseActions"></a>Usando ações de Mouse: Arrastando itens de compartimento  
