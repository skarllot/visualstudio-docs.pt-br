---
title: 'Como: acessar o esquema de cores e fontes internas | Documentos do Microsoft'
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
ms.openlocfilehash: fa738450b3e792946af7f55ec8aa65e668e2a3e1
...
-   Informar o IDE que uma janela específica usa os itens de exibição interna e as categorias usando o `T:Microsoft.VisualStudio.TextManager.Interop.IVsTextEditorPropertyCategoryContainer` e `T:Microsoft.VisualStudio.TextManager.Interop.IVsTextEditorPropertyContainer` interfaces.  
  
 O IDE usa a categoria resultante como um identificador para a janela. Nome da categoria é exibido no **Mostrar configurações de:** caixa suspensa a **fontes e cores** página de propriedades.  
...
2.  Nome da categoria deve ser adicionado a uma tabela de cadeia de caracteres dentro do arquivo de recursos (RC) do VSPackage, para que ele pode ser localizado conforme necessário quando exibido no IDE.  
     ...
     Para obter mais informações, consulte [adicionando ou excluindo uma cadeia de caracteres](/visual-cpp/windows/adding-or-deleting-a-string).  
...
2.  Preencha o registro para usar as fontes de estoque e o esquema de cores com quatro valores:  
    ...
    |----------|----------|----------|-----------------|  
    |Categoria|REG_SZ|GUID|Um GUID arbitrária que identifica uma categoria que contém o esquema de fontes e cores de estoque.|  
    ...
    |NameID|REG_DWORD|ID|A ID de recurso de um nome de categoria localizáveis VSPackage.|  
    |ToolWindowPackage|REG_SZ|GUID|O GUID da implementação de VSPackage o <xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextView>interface.</xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextView>|  
  
3.  
  
### <a name="to-initiate-the-use-of-system-provided-fonts-and-colors"></a>Para iniciar o uso do sistema forneceu fontes e cores  
  
1.  Criar uma instância do `T:Microsoft.VisualStudio.TextManager.Interop.IVsTextEditorPropertyCategoryContainer` interface como parte da implementação e inicialização da janela.  
  
2.  Chamar o <xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextEditorPropertyCategoryContainer.GetPropertyCategory%2A>método para obter uma instância de `T:Microsoft.VisualStudio.TextManager.Interop.IVsTextEditorPropertyContainer` interface correspondente a atual <xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextView>instância.</xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextView> </xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextEditorPropertyCategoryContainer.GetPropertyCategory%2A>  
