...
Não são permitidas restrições em declarações não genéricas  
 ...
 A sintaxe encontrada só pode ser usada em uma declaração genérica para aplicar restrições para o parâmetro de tipo. Para obter mais informações, consulte [Genéricos](/dotnet/csharp/programming-guide/generics/index).  
