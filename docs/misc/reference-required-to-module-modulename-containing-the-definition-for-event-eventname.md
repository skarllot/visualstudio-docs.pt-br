...
Referência necessária para o módulo ' \<`modulename`\>' contendo a definição para evento ' \<`eventname`\>'. Adicione uma ao seu projeto.  
 ...
 O evento é definido em um módulo que não é referenciado diretamente em seu projeto. O [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] compilador requer uma referência para evitar ambigüidade, caso o evento é definido em mais de um módulo.  
